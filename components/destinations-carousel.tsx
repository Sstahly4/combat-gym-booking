...
export function DestinationsCarousel({ destinations }: DestinationsCarouselProps) {
  ...
  const scroll = (direction: 'left' | 'right') => {
    if (scrollRef.current) {
      ...
      // On mobile: 4 cards per view (25vw), on desktop: 3 cards per view
      ...
      >
        {destinations.map((city) => (
          <Link
            ...
            className="min-w-[calc(25%-9px)] md:min-w-[calc(33.333%-10.67px)] max-w-[calc(25%-9px)] md:max-w-[calc(33.333%-10.67px)] snap-start flex-shrink-0"
          >
