...
export function MobileBottomNav() {
  ...
  if (isHiddenRoute(pathname) || reviewChromeHidden) return null
        ...
        <div
          className={`mx-auto flex items-center transition-[max-width,gap] duration-300 ease-out ${
            useWideTabLayout
              ...
              : 'max-w-[22rem] justify-center gap-[1.2rem]'
          }`}
        >
          {items.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex min-w-0 flex-col items-center justify-center gap-0.5 rounded-xl px-2 py-1.5 text-center text-[11px] leading-tight tracking-tight transition-colors active:bg-blue-50 ${
                  useWideTabLayout ? 'flex-1' : 'w-[4.875rem]'
                } ${activeClass}`}
              >
                <span
                  key={iconPop?.href === item.href ? String(iconPop.token) : 'idle'}
                  className={`inline-flex h-[23.5px] w-[23.5px] shrink-0 items-center justify-center origin-center will-change-transform ${
                ...
                >
                  <Icon className="h-[23.5px] w-[23.5px]" strokeWidth={item.active ? 2.05 : 1.75} aria-hidden />
                </span>
