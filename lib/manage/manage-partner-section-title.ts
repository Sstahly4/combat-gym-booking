...
export function managePartnerSectionTitle(pathname: string): string {
  ...
  if (pathname.startsWith('/manage/upcoming-bookings')) return 'Bookings'
  ...
  if (pathname.startsWith('/manage/balances/payouts')) return 'Payouts'
  if (pathname.startsWith('/manage/balances')) return 'Balances'
  if (pathname.startsWith('/manage/payouts/setup')) return 'Payouts'
